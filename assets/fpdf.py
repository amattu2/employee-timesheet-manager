...
import math
import errno
import os, sys, zlib, struct, re, tempfile, struct, re
...
def phone_format(number):
	...
	return re.sub("(\d)(?=(\d{3})+(?!\d))", r"\1-", "%d" % int(number[:-1])) + number[-1]

def date_format(string, input_format = "%m/%d/%Y %H:%M", output_format = "%b {ds}, %Y"):
	# Variables
	input_datetime = datetime.strptime(string, input_format)
	input_suffix = "th" if (11 <= input_datetime.day <= 13) else {1: 'st', 2: 'nd', 3: 'rd'}.get(input_datetime.day%10, 'th')
	output_datetime = input_datetime.strftime(output_format)
	output_datetime = output_datetime.replace('{ds}', str(input_datetime.day) + str(input_suffix))
...
class FPDF(object):
    ...
    def set_producter(self, producer):
...
#            self.error("Don\'t alter the locale before including class file");
    ...
    def _putinfo(self):
        if hasattr(self, 'producer'):
            self._out('/Producer '+self._textstring(self.producer))
        else:
            self._out('/Producer '+self._textstring("N/A"))
